...
INSERT INTO users_payment VALUES
...
CREATE TABLE transactions_payment (
    ...
    txn_date DATE,
    FOREIGN KEY (user_id) REFERENCES users_payment(user_id)
);

INSERT INTO transactions_payment VALUES
...
CREATE TABLE risk_flags_payment (
...
INSERT INTO risk_flags_payment VALUES
...
SELECT * FROM transactions_payment;
SELECT * FROM risk_flags_payment;

---------------------------------------------------------------------------------------------------------
-- Q1. Users with more than 1 FAILED transaction
SELECT user_id, COUNT(*)
FROM transactions_payment
WHERE status = 'FAILED'
GROUP BY user_id
HAVING COUNT(*) > 1;
...
FROM transactions_payment tp
INNER JOIN risk_flags_payment rp
...
FROM risk_flags_payment rp
LEFT JOIN transactions_payment tp
